...
Créer une variable globale `SERVER_BASE_URL` initialisée à `http://localhost:5000` pour :
...
// Résultat: http://localhost:5000/api/users

const portfolioUrl = buildServerUrl('portfolio', 'image.jpg');
// Résultat: http://localhost:5000/uploads/portfolio/image.jpg
```
...
await fetchAndMergeGlobalData('http://localhost:5000', 'demo-11');
...
#### Développement Local
```javascript
setServerBaseUrl('http://localhost:5000'); // Serveur principal
